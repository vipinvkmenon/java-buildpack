...
module JavaBuildpack
  module Coprocess
    ...
    class JaegerAgent < JavaBuildpack::Component::BaseComponent
      ...
      def detect
        @logger.debug("Detecting CoProcess---------------------------------------")
      ...
      def compile
        @logger.debug("Compiling CoProcess---------------------------------------")
       # download_tar("1.16.0","https://github.com/jaegertracing/jaeger/releases/download/v1.16.0/jaeger-1.16.0-linux-amd64.tar.gz")
        ...
        FileUtils.rm_rf(@droplet.sandbox)
         ...
         #./jaeger-agent --reporter.grpc.tls=true --reporter.grpc.tls.ca=tls_cert.crt --reporter.grpc.tls.cert=tls_cert.crt --reporter.grpc.tls.key=tls_key.key --reporter.grpc.host-port=arpit.ingress.mon-team.perf-load.shoot.dev.k8s-hana.ondemand.com:443
        
      end

      # (see JavaBuildpack::Component::BaseComponent#release)
      def release
       @logger.debug("Releasing CoProcess----------------------------------------")
